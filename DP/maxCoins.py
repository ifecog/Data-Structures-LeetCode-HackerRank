...
# dp[i][k][l] = max coins at mile i, using k switches so far, and currently in lane l
# i = mile_index(0 to n - 1)
# k = number of switches used so far (0, 1, 2)
# ill = current_lane (0 for lane1, 1 for lane2)

# You can switch lanes if we still have switches left (k > 0)
    ...
    for i in range(1, n):
        for k in range(3):
            for l in range(2):
                ...
                # Stay on the sane lane
                dp[i][k][l] = max(dp[i][k][l], dp[i - 1][k][l] + val)
                
                # Swith from other lane if switch is available
                if k > 0:
                    dp[i][k][l] = max(
                        dp[i][k][l], dp[i - 1][k - 1][l - 1] + val
                    )   
